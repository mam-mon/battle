...
class CombatScreen(BaseState):
    ...
    def _on_victory(self):
        ...
        # 检查状态栈下方是否是 DungeonScreen
        if len(self.game.state_stack) > 1:
            dungeon_screen = self.game.state_stack[-2]
            if isinstance(dungeon_screen, DungeonScreen) and self.origin_id:
                # <-- 核心改动：调用新方法，汇报被击败怪物的 uid -->
                dungeon_screen.on_monster_defeated(self.origin_id)

        # 进入战利品结算界面
        from .loot import LootScreen
        self.game.state_stack.pop()
        self.game.state_stack.append(LootScreen(self.game, self.enemy_id))
