...
class DungeonScreen(BaseState):
    def __init__(self, game):
        super().__init__(game)
        self.floor = Floor()
        self.floor.generate_floor(num_rooms=8) # <-- 已修改为8
    ...
    def _enter_room(self, room):
        ...
        if not (self.current_room.type == 'boss' and self.current_room.is_cleared): self.exit_portal_button = None
        print(f"进入房间 ({room.x}, {room.y}), 类型: {room.type}")
        self._sync_sprites(); self.door_rects = self._generate_doors()
    ...
    def _sync_sprites(self):
        self.monster_group.empty(); self.chest_group.empty()
        if not self.current_room.is_cleared:
            if self.current_room.type in ["combat", "elite", "boss"]:
                for monster_data in self.current_room.monsters: self.monster_group.add(Monster(monster_data))
            elif self.current_room.type == "treasure": self.chest_group.add(TreasureChest(SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2))
    def _open_treasure_chest(self, chest_sprite):
        ...
        if len(all_item_classes) < 2: print("错误：装备池不足，无法生成两个选项"); return
        choices = random.sample(all_item_classes, 2); item1, item2 = choices[0](), choices[1]()
        from .choice_screen import ChoiceScreen
        self.game.state_stack.append(ChoiceScreen(self.game, [item1, item2], self.current_room)); chest_sprite.kill()
    def handle_event(self, event):
        from .backpack import BackpackScreen
        if self.backpack_button.handle_event(event) or (event.type == pygame.KEYDOWN and event.key == pygame.K_b):
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
            ...
            if chest and chest.rect.collidepoint(event.pos): self._open_treasure_chest(chest)
    ...
    def update(self):
        ...
        if not self.current_room.is_cleared:
            ...
            for direction, door_rect in self.door_rects.items():
                if self.player.rect.colliderect(door_rect): self._change_room(direction); break
    ...
    def draw(self, surface):
        ...
        self.monster_group.draw(surface); self.chest_group.draw(surface); self.player_group.draw(surface)
    ...
    def _draw_minimap(self, surface):
        ...
        for (x, y), room in self.floor.rooms.items():
            ...
            if not room.is_cleared and room.type != "start": final_color = (base_color[0] // 2, base_color[1] // 2, base_color[2] // 2)
